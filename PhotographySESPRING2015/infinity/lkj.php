            ...
            if ($revChoice['S3_id'] == 'x') {
                ...
                while ($sessionresults = mysql_fetch_assoc($revresults)) {
                    ...
                    if ($inrowsum < 7)
					{
						...
						if ($inrow[4] != 1) {
							$insert = "UPDATE `session` SET Time_5 = $row[Id] WHERE Rev_id = $sessionresults[Rev_id]  AND S_id = 'S3' AND Time_5 IS NULL AND S_id LIKE '$row[Day]3'";
							mysql_query($insert);
							if (mysql_affected_rows() == 1)
								goto endofwhile;
						}
						...
						if ($inrow[5] != 1) {
							$insert = "UPDATE `session` SET Time_6 = $row[Id] WHERE Rev_id = $sessionresults[Rev_id]  AND S_id = 'S3' AND Time_6 IS NULL AND S_id LIKE '$row[Day]3'";
							mysql_query($insert);
							if (mysql_affected_rows() == 1)
								goto endofwhile;
						}
